...
use http::HeaderMap;
use object_store::{
    Attributes, CopyOptions, Error as o_Error, GetOptions, GetResult, ListResult, MultipartUpload,
...
impl ObjectStore for EcObjectStore {
    ...
    fn delete_stream(
        ...
        // EcObjectStore is read-only; return an error for every path.
        locations
            .map(|r| r.and_then(|_| Err(unsupported_store_error("delete_stream"))))
            .boxed()
    }
